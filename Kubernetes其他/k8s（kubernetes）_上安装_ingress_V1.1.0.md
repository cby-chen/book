...
layout: post
cid: 65
title: 在k8s（kubernetes） 上安装 ingress V1.1.0
slug: 65
date: 2021/12/30 17:14:31
updated: 2021/12/30 17:14:31
status: publish
author: cby
categories: 
  - 默认分类
...
![图片](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e05c7cbd7ca6401b8765d7d737028364~tplv-k3u1fbpfcp-zoom-1.image)

  

# 在使用  ingress 创建后发现没有默认HTTP

  

```
[root@hello ~/yaml/nginx]# kubectl  describe ingress
...
Rules:
  ...
  ----             ----  --------
  hello.chenby.cn  
                   /   hello-server:8000 (172.20.1.13:9000,172.20.1.14:9000)
  demo.chenby.cn  
                   /nginx   nginx-demo:8000 (172.20.2.14:80,172.20.2.15:80)
Annotations:       <none>
Events:
  Type    Reason  Age   From                      Message
  ----    ------  ----  ----                      -------
  Normal  Sync    43m   nginx-ingress-controller  Scheduled for sync
[root@hello ~/yaml/nginx]#
```
...
出现该问题后是因为没有创建默认的后端，需要卸载之前安装的，之前用什么方式安装就用对应的方式卸载
